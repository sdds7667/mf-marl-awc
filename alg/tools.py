...
class EpisodesBufferEntry:
    ...
    def append(self, view, feature, action, reward, probs=None):
...
class EpisodesBuffer(Buffer):
    ...
    def push(self, **kwargs):
        ...
        for i in range(len(ids)):
            ...
            if self.use_mean:
                entry.append(view[i], feature[i], acts[i], rewards[i],  probs=probs[i])
            else:
...
class AgentMemory(object):
    def __init__(self, obs_shape, feat_shape, act_n, max_len, former_action_shape, use_mean=False):
        ...
        if self.use_mean:
            # 3, 3 - radius 1,1
            self.prob = MetaBuffer(former_action_shape, max_len)

    def append(self, obs0, feat0, act, reward, prob=None):
...
class MemoryGroup(object):
    def __init__(self, obs_shape, feat_shape, act_n, max_len, batch_size, sub_len, former_actions=None, use_mean=False):
        self.agent = dict()
        self.max_len = max_len
        ...
        self.use_mean = use_mean
        self.former_actions = former_actions
        self.act_n = act_n
        ...
        self.terminals = MetaBuffer((), max_len, dtype='bool')
        if use_mean:
            self.prob = MetaBuffer(former_actions, max_len)
        self._new_add = 0
    ...
    def push(self, **kwargs):
        for i, _id in enumerate(kwargs['ids']):
            if self.agent.get(_id) is None:
                self.agent[_id] = AgentMemory(self.obs_shape, self.feat_shape, self.act_n, self.sub_len, self.former_actions,
                                              use_mean=self.use_mean)
            if self.use_mean:
                self.agent[_id].append(obs0=kwargs['state'][0][i], feat0=kwargs['state'][1][i], act=kwargs['acts'][i],
                                       reward=kwargs['rewards'][i], prob=kwargs['prob'][i])
            else:
                self.agent[_id].append(obs0=kwargs['state'][0][i], feat0=kwargs['state'][1][i], act=kwargs['acts'][i],
    ...
    def sample(self):
        idx = np.random.choice(self.nb_entries-1, size=self.batch_size)
        next_idx = idx + 1
        ...
        if self.use_mean:
            ...
            return obs, feature, actions, act_prob, obs_next, feature_next, act_next_prob, rewards,
        else:
            return obs, feature, obs_next, feature_next, rewards, actions
...
class SummaryObj:
    ...
    def __init__(self, log_dir, log_name, n_group=1):
        ...
        with self.gra.as_default():
            self.sess = tf.compat.v1.Session(graph=self.gra, config=sess_config)
            self.train_writer = tf.compat.v1.summary.FileWriter(log_dir + "/" + log_name,
    ...
    def register(self, name_list):
        ...
        with self.gra.as_default():
            for name in name_list:
                ...
                setattr(self, name,
                        [tf.compat.v1.placeholder(tf.compat.v1.float32, shape=None, name='Agent_{}_{}'.format(i, name))
                         for i in range(self.n_group)])
                setattr(self, name + "_op",
                        [tf.compat.v1.summary.scalar('Agent_{}_{}_op'.format(i, name), getattr(self, name)[i])
                         for i in range(self.n_group)])
...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, model,
        ...
        if self.train:
            ...
            l_vars = self.model.vars
            self.sp_op = [tf.compat.v1.assign(l_vars[i], (1. - tau) * l_vars[i])
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        max_nums, nums, agent_r_records, total_rewards = self.play(env=self.env, n_round=iteration,
                                                                   ...
                                                                   model=self.model, print_every=50, eps=variant_eps,
                                                                   render=(
                                                                                      iteration + 1) % self.render_every if self.render_every > 0 else False,
                                                                   train=self.train)
        ...
        if self.train:
            ...
            if info['main']['total_reward'] > info['opponent']['total_reward']:
                ...
                self.model.save(self.model_dir + '-0', iteration)
